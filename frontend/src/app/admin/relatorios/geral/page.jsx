...
export default function RelatorioGeral() {
  ...
  const COLORS = ['#76196c', '#9b2b91', '#d695e7', '#75ba51', '#4f6940', '#e8c5f1', '#f5e0f9'];
  ...
  const prepararDadosFiliais = () => {
    ...
    return data.rankingFiliais
      ...
      .map(item => ({
        nome: item.nome_filial.length > 12 ? item.nome_filial.substring(0, 12) + '...' : item.nome_filial,
        vendas: parseInt(item.total_vendas),
  ...
  const prepararDadosStatusFiliais = () => {
    ...
    return [
      { name: 'Ativas', value: parseInt(data.empresas.filiais_ativas), color: '#75ba51' },
      { name: 'Inativas', value: parseInt(data.empresas.filiais_inativas), color: '#d695e7' }
    ];
  };
    ...
    return (
      <div className="min-h-screen p-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[#76196c] font-medium">Carregando relatório geral...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="text-center mb-6">
            ...
            {data?.periodo && (
              <p className="text-sm text-neutral-500 mt-2">
                Período: {new Date(data.periodo.dataInicio).toLocaleDateString('pt-BR')} até {new Date(data.periodo.dataFim).toLocaleDateString('pt-BR')}
              </p>
            )}
          </div>
            ...
            <div className="overflow-x-auto mt-2">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-gradient-to-r from-[#f8f0fc] to-[#fef5ff] border-b-2 border-[#d695e7]">
                    <th className="p-4 text-left text-[#76196c] font-bold">ID</th>
                    <th className="p-4 text-left text-[#76196c] font-bold">Descrição</th>
                    <th className="p-4 text-left text-[#76196c] font-bold">Valor</th>
                    <th className="p-4 text-left text-[#76196c] font-bold">Data</th>
                  </tr>
                </thead>
                <tbody>
                  {data.topDespesas.map((d) => (
                    <tr key={d.id_despesa} className="border-b hover:bg-[#fef5ff] transition-colors">
                      <td className="p-4">
                        <span className="font-mono text-sm bg-[#e8c5f1] text-[#76196c] px-3 py-1 rounded-lg">
                          #{d.id_despesa}
                        </span>
                      </td>
                      <td className="p-4 text-sm max-w-md truncate" title={d.descricao}>
                        {d.descricao}
                      </td>
                      <td className="p-4 font-bold text-red-600">
                        R$ {parseFloat(d.preco).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </td>
                      <td className="p-4 text-neutral-600">
          ...
          <Card className="shadow-lg border-2 border-[#d695e7] bg-white">
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm text-neutral-500 font-medium">Total de Filiais</p>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h2 className="text-2xl font-bold text-green-600">
                    R$ {data?.financeiro?.faturamento?.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) || '0,00'}
                  </h2>
                  <p className="text-xs text-neutral-400 mt-1">
                    Lucro: R$ {data?.financeiro?.lucroGeral?.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-neutral-400 mt-1">
                    Ticket: R$ {data?.financeiro?.ticketMedio?.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <Card className="shadow-lg border-2 border-[#d695e7] bg-white">
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm text-neutral-500 font-medium">Produtos</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <Card className="shadow-lg border-2 border-[#d695e7] bg-white">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-neutral-500 font-medium">Saídas/Despesas</p>
                  <h2 className="text-2xl font-bold text-red-500">
                    R$ {data?.financeiro?.totalSaidas?.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) || '0,00'}
                  </h2>
                </div>
          ...
          <Card className="shadow-lg border-2 border-[#d695e7] bg-white">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-neutral-500 font-medium">Margem de Lucro</p>
          ...
          <Card className="shadow-lg border-2 border-[#d695e7] bg-white">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-neutral-500 font-medium">Produtos Baixos</p>
                ...
                <AreaChart data={prepararDadosVendasPorDia()}>
                  <defs>
                    <linearGradient id="colorVendas" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#76196c" stopOpacity={0.8}/>
                      <stop offset="95%" stopColor="#76196c" stopOpacity={0.1}/>
                    </linearGradient>
                    <linearGradient id="colorFaturamento" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#75ba51" stopOpacity={0.8}/>
                      <stop offset="95%" stopColor="#75ba51" stopOpacity={0.1}/>
                    </linearGradient>
                  </defs>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-xl border-2 border-[#d695e7] lg:col-span-2 space-y-4">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={prepararDadosTopProdutos()} layout="vertical">
                  <CartesianGrid strokeDasharray="3 3" stroke="#e8c5f1" />
                  <XAxis type="number" stroke="#76196c" style={{ fontSize: '12px' }} />
                  <YAxis dataKey="nome" type="category" stroke="#76196c" style={{ fontSize: '12px' }} width={120} />
                  <Tooltip 
                    contentStyle={{ 
                      backgroundColor: '#fff', 
                      border: '2px solid #d695e7',
                  ...
                  />
                  <Legend />
                  <Bar dataKey="quantidade" fill="#76196c" name="Quantidade Vendida" radius={[0, 8, 8, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-gradient-to-r from-[#f8f0fc] to-[#fef5ff] border-b-2 border-[#d695e7]">
                    ...
                    <th className="p-4 text-left text-[#76196c] font-bold">Vendas</th>
                    <th className="p-4 text-left text-[#76196c] font-bold">Faturamento</th>
                    <th className="p-4 text-left text-[#76196c] font-bold">Status</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={f.id_empresa} className="border-b hover:bg-[#fef5ff] transition-colors">
                      ...
                      <td className="p-4">
                        <span className={`px-3 py-1 rounded-lg text-sm font-medium ${
                          ...
                          {parseFloat(f.faturamento) > 0 ? 'Ativa' : 'Inativa'}
                        </span>
                      </td>
                    </tr>
                  ))}
                ...
                <tbody>
                  {data.ultimasDespesas.slice(0, 15).map((d) => (
                    <tr key={d.id_despesa} className="border-b hover:bg-[#fef5ff] transition-colors">
                      ...
                      <td className="p-4 text-neutral-600">
                        {d.data_pag ? new Date(d.data_pag).toLocaleDateString('pt-BR') : '-'}
                      </td>
                    </tr>
                  ))}
                </tbody>
