...
export default function AdminRelatorios() {
  ...
  const buscarRelatorio = async () => {
    ...
    try {
      ...
      console.log("[ADMIN RELATORIOS] dados recebidos:", data.retorno);
    } catch (error) {
  ...
  // debug: log whenever filtros change (helps trace UI interactions)
  useEffect(() => {
    console.debug("[ADMIN RELATORIOS] filtros alterados", filtros);
  }, [filtros]);
                    ...
                    <tbody>
                      {dados.gastos.map((g) => (
                        <tr key={g.id_despesa} className="border-b">
                          ...
                          <td className="p-3">
                            {new Date(g.data_adicionado).toLocaleDateString(
                              "pt-BR"
                            )}
                          </td>
