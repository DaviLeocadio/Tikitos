...
export default function ModalDesativarGerente({
  gerente,
  open,
  onClose,
  onSalvar,
  ...
  const isInativo = gerente?.status === "inativo";

  const handleConfirmar = async () => {
    const idGerente = gerente?.id_usuario;
    if (!idGerente) {
      aparecerToast("Gerente inválido!");
      return;
    }
    ...
    try {
      const endpoint = isInativo
        ? `http://localhost:8080/admin/gerentes/${gerente.id_usuario}/ativar`
        : `http://localhost:8080/admin/gerentes/${gerente.id_usuario}/desativar`;
      ...
      if (response.ok) {
        aparecerToast(
          isInativo
            ? "Gerente reativado com sucesso!"
            : "Gerente desativado com sucesso!"
        );
        await onSalvar();
        onClose();
      } else {
        const error = await response.json();
        aparecerToast(error.error || "Erro ao atualizar status do Gerente!");
      }
    } catch (error) {
      console.error("Erro ao atualizar status:", error);
      aparecerToast("Erro ao atualizar status do Gerente!");
    } finally {
      setLoading(false);
    }
  };
    ...
    <AlertDialog open={open} onOpenChange={onClose}>
      ...
      >
        <AlertDialogHeader>
          ...
          >
            {isInativo ? "Reativar Gerente?" : "Desativar Gerente?"}
          </AlertDialogTitle>
        </AlertDialogHeader>
