...
const DashedCard = ({ title, children, icon: Icon }) => (
  <div
    className="bg-white w-full p-6 rounded-xl shadow-md transition duration-300 border-2 border-dashed"
    style={{ borderColor: TIKITOS_COLORS.light }}
  >
    <header
      className="flex items-center mb-4 pb-3 border-b border-dashed"
      style={{ borderColor: TIKITOS_COLORS.light }}
    >
      {Icon && (
        <Icon
...
const FinanceMetric = ({
  ...
  <div className="flex items-center space-x-3 p-3 rounded-lg shadow-sm border border-gray-100">
    ...
    <div>
      ...
      <p
        className={`font-extrabold ${isLarge ? "text-2xl" : "text-lg"}`}
        style={{ color: color }}
      >
        {value}
      </p>
...
const CaixaList = ({ title, data, color }) => (
  <div>
    <h4 className="font-semibold text-base mb-2" style={{ color }}>
      {title}
    </h4>
    <div className="space-y-2">
      {data && data.length > 0 ? (
        data.slice(0, 3).map((mov, index) => (
          <div
            key={index}
            className="flex justify-between text-sm text-gray-700 bg-gray-50 p-2 rounded-md border border-gray-200"
          >
            <span>{formatDate(mov.data, false)}</span>
...
const LojaInfoCard = ({ loja }) => {
  const statusColor =
    loja.status === "ativo" ? TIKITOS_COLORS.success : "rgb(239 68 68)";
  return (
    <DashedCard title="Dados Gerais da Loja" icon={Briefcase}>
      <DetailItem
        ...
        value={loja.nome}
        icon={ShoppingCart}
        color={TIKITOS_COLORS.medium}
      />
      <DetailItem
        label="Tipo"
        value={loja.tipo?.toUpperCase?.() || ""}
        icon={ClipboardList}
        color={TIKITOS_COLORS.medium}
      />
      <DetailItem
      ...
      <DetailItem
        label="Gerente Atual"
        value={loja.gerente?.nome || "Não atribuído"}
        icon={User}
        color="rgb(37 99 235)"
      />
      <DetailItem
        label="Status Operacional"
        value={loja.status?.toUpperCase?.() || ""}
        icon={loja.status === "ativo" ? ShieldCheck : Zap}
        color={statusColor}
      />
    </DashedCard>
  );
...
const GerenteVendedoresCard = ({ gerente, vendedores }) => (
  <DashedCard title="Equipe de Vendas" icon={Users}>
    <h3 className="text-lg font-bold" style={{ color: TIKITOS_COLORS.medium }}>
      Gerente Responsável:
    </h3>
    <div
      className="flex items-center space-x-4 p-3 rounded-lg"
      style={{ backgroundColor: TIKITOS_COLORS.light }}
    >
      <div
        className="w-12 h-12 bg-gray-300 rounded-full flex items-center justify-center text-gray-600 font-bold text-lg flex-shrink-0"
        style={{ color: TIKITOS_COLORS.primary }}
      >
    ...
    <h3
      ...
      style={{ color: TIKITOS_COLORS.medium }}
    >
      Lista de Vendedores:
    </h3>
      ...
      {vendedores && vendedores.length > 0 ? (
        vendedores.map((vendedor) => (
          <div
            ...
            className="p-3 bg-white rounded-lg shadow-sm flex items-center space-x-2 border border-gray-100"
          >
...
const FinancasCard = ({ financeiro }) => {
  const { faturamento_mes, total_vendas_mes, ticket_medio, ultima_venda } =
    financeiro || {};
  return (
    <DashedCard title="Resumo Financeiro Mensal" icon={DollarSign}>
      <FinanceMetric
        label="Faturamento do Mês"
        value={formatCurrency(Number(faturamento_mes || 0))}
        icon={Banknote}
        color={TIKITOS_COLORS.success}
        isLarge
      />
      <FinanceMetric
        label="Total de Vendas"
        value={Number(total_vendas_mes || 0)}
        icon={TrendingUp}
        color={TIKITOS_COLORS.medium}
      />
      <FinanceMetric
        label="Ticket Médio"
        value={formatCurrency(Number(ticket_medio || 0))}
        icon={DollarSign}
        color={TIKITOS_COLORS.highlight}
      />
      <FinanceMetric
        label="Última Venda"
        value={formatDate(ultima_venda)}
        icon={Clock}
...
const EstoqueCard = ({ estoque }) => {
  const isCritical =
    estoque?.estoque_status === "BAIXO" ||
    (estoque?.produtos_criticos || 0) > 0;
  ...
  return (
    <DashedCard title="Situação do Estoque" icon={Package}>
      <div className="flex justify-between items-center pb-3 border-b border-gray-100">
        ...
        >
          ...
          <span className="animate-pulse">
            {React.createElement(statusIcon, { size: 20 })}
          </span>
        </span>
      ...
      <DetailItem
        ...
        value={estoque?.produtos_criticos}
        icon={Zap}
        color="rgb(239 68 68)"
      />
      <p
      ...
      <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
        {estoque?.produtos && estoque.produtos.length > 0 ? (
          estoque.produtos.map((p) => (
            <div
              key={p.id_produto}
              className="flex justify-between text-sm p-2 rounded-lg bg-gray-50 border border-dashed border-gray-200"
            >
...
const VendasCard = ({ ultimas_vendas }) => (
  <DashedCard title="Últimas Vendas" icon={Clock}>
    <div className="space-y-3 w-full max-h-76 overflow-y-auto pr-2">
      {ultimas_vendas && ultimas_vendas.length > 0 ? (
        ultimas_vendas.map((venda, index) => (
          <div
            key={index}
            className="flex items-center justify-between p-3 rounded-lg border-l-4 shadow-sm"
            style={{
              borderColor: TIKITOS_COLORS.highlight,
              backgroundColor: TIKITOS_COLORS.light + "1A",
            }}
          >
            <div>
              <p className="font-semibold text-gray-800 text-sm">
                {formatCurrency(parseFloat(venda.total) || 0)}
              </p>
              <p className="text-xs text-gray-500">
                Venda #{venda.id_venda} por {venda.vendedor}
              </p>
            </div>
            <p className="text-xs font-medium text-gray-600">
              {formatDate(venda.data_venda, false)}
            </p>
          </div>
        ))
      ) : (
    ...
    <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
      {despesas && despesas.length > 0 ? (
        despesas.map((despesa, index) => (
          <div
            key={index}
            className="flex justify-between items-start p-3 rounded-lg border-l-4 shadow-sm bg-red-50 border-red-300"
          >
...
const LojaHeader = ({ lojaName, status, loading, handleRefresh }) => {
  ...
  return (
    <header
      className="mb-8 border-b-2 border-dashed pb-4 flex flex-col md:flex-row justify-between items-start md:items-center"
      style={{ borderColor: TIKITOS_COLORS.light }}
    >
      <div className="flex items-center mb-4 md:mb-0">
        <h1
          className="text-3xl font-extrabold mr-4"
          style={{ color: TIKITOS_COLORS.primary }}
        >
          {lojaName}
        </h1>
        <span
          className="px-4 py-1 text-sm font-bold rounded-full uppercase text-white"
      ...
      <div className="flex space-x-3">
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center px-4 py-2 text-white font-medium rounded-lg shadow-md transition duration-150 text-sm disabled:opacity-50"
...
export default function LojaDetalhesComponent({ params }) {
  ...
  const fetchLojaDetails = useCallback(
    async (retries = 3) => {
      setLoading(true);
      setError(null);
      let lastError = null;
      if (lojaId === 200) {
        ...
        setLoja(null); // allow mock usage in original dev file
      }
      ...
      return (
        <div className="text-center py-20 flex flex-col items-center">
          <RefreshCw
            ...
            style={{ color: TIKITOS_COLORS.highlight }}
          />
          <p
            className="text-xl font-medium"
            style={{ color: TIKITOS_COLORS.medium }}
          >
            Carregando detalhes da filial {lojaId}...
          </p>
        </div>
      );
      ...
      return (
        <div className="text-center py-20 p-8 bg-red-50 border border-red-300 rounded-lg shadow-md">
          ...
          <p className="text-red-600 mb-4">
            Ocorreu um problema ao buscar os dados da loja:
          </p>
          <pre className="p-3 bg-red-100 rounded-md text-sm text-left whitespace-pre-wrap">
            {error || "Sem dados retornados"}
          </pre>
          <button
            onClick={() => fetchLojaDetails(1)}
            className="mt-6 px-6 py-2 text-white font-medium rounded-lg shadow-md transition duration-150"
  ...
  return (
    <div className="min-h-screen p-4 sm:p-8 lg:p-12">
      <div className="max-w-7xl mx-auto">
        <LojaHeader
          lojaName={loja?.nome || `Loja #${lojaId}`}
