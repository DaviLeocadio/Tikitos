...
export default function ModalDesativarProduto({
  produto,
  open,
  onClose,
  onSalvar,
  ...
  const handleConfirmar = async () => {
    ...
    try {
      const endpoint = isInativo
        ? `http://localhost:8080/admin/produtos/${produto.id_produto}/ativar`
        : `http://localhost:8080/admin/produtos/${produto.id_produto}/desativar`;
      ...
      if (response.ok) {
        ...
        aparecerToast(
          error.error || "Erro ao atualizar status do produto!"
        );
      }
    } catch (error) {
      console.error("Erro ao atualizar status:", error);
      aparecerToast("Erro ao atualizar status do produto!");
    } finally {
      setLoading(false);
    }
  };
    ...
    <AlertDialog open={open} onOpenChange={onClose}>
      ...
      >
        <AlertDialogHeader>
          ...
          >
            {isInativo ? "Reativar produto?" : "Desativar produto?"}
          </AlertDialogTitle>
        </AlertDialogHeader>
        ...
        <div className="space-y-3">
          <div className="bg-white rounded-lg p-4 border-2 border-dashed border-gray-300">
            ...
            <p className="text-xs text-gray-500 mt-1">{produto?.status}</p>
          </div>
          ...
          >
            <div className="flex gap-2">
              ...
              <p className="text-sm font-semibold text-gray-700">
                {isInativo
                  ? "O produto está de volta"
                  : "O produto não está de volta"}
              </p>
            </div>
          </div>
        </div>
