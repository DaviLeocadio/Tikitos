...
const obterResumoVendas = async (idEmpresa, periodo, idFilial = null) => {
  ...
  const filialFilter = idFilial ? `AND id_filial = ?` : "";
...
const obterVendasPorFilial = async (idEmpresa, periodo) => {
  const sql = `
    SELECT f.id_filial, f.nome as nome_filial, COUNT(v.id_venda) as total_vendas, COALESCE(SUM(v.total), 0) as valor_total
    FROM vendas v INNER JOIN filiais f ON v.id_filial = f.id_filial
    WHERE v.id_empresa = ? AND ${getWhereData(periodo, 'data_venda', 'v')}
    GROUP BY f.id_filial, f.nome ORDER BY valor_total DESC`;
  return await execQuery(sql, [idEmpresa]);
};

const obterContasAPagar = async (idEmpresa, idFilial = null) => {
  const filter = idFilial ? `AND d.id_filial = ?` : "";
  const params = idFilial ? [idEmpresa, idFilial] : [idEmpresa];
  const sql = `
...
const obterSaldoConsolidado = async (idEmpresa, idFilial = null) => {
  const filter = idFilial ? `AND id_filial = ?` : "";
  const params = idFilial ? [idEmpresa, idFilial] : [idEmpresa];
...
const listarFiliais = (id) => execQuery(`SELECT * FROM filiais WHERE id_empresa = ? AND status = 'ativa' ORDER BY nome`, [id]);

const criarFilial = (idEmpresa, d) => execQuery(
  `INSERT INTO filiais (id_empresa, nome, endereco, telefone, email, cidade, estado, status, data_criacao) VALUES (?, ?, ?, ?, ?, ?, ?, 'ativa', NOW())`,
  [idEmpresa, d.nome, d.endereco, d.telefone, d.email, d.cidade, d.estado]
);

const atualizarFilial = (id, d) => execQuery(
  `UPDATE filiais SET nome=?, endereco=?, telefone=?, email=?, cidade=?, estado=?, data_atualizacao=NOW() WHERE id_filial=?`,
  [d.nome, d.endereco, d.telefone, d.email, d.cidade, d.estado, id]
);

const desativarFilial = (id) => execQuery(`UPDATE filiais SET status='inativa', data_atualizacao=NOW() WHERE id_filial=?`, [id]);
...
const listarUsuarios = (idEmpresa, idFilial = null) => {
  const filter = idFilial ? `AND u.id_filial = ?` : "";
  const params = idFilial ? [idEmpresa, idFilial] : [idEmpresa];
  return execQuery(`
    SELECT u.id_usuario, u.nome, u.email, u.perfil, u.status, f.nome as nome_filial, u.data_criacao
    FROM usuarios u LEFT JOIN filiais f ON u.id_filial = f.id_filial
    WHERE u.id_empresa = ? ${filter} ORDER BY u.nome`, params);
};

const transferirFuncionario = (idUsuario, idFilial) => execQuery(`UPDATE usuarios SET id_filial=?, data_atualizacao=NOW() WHERE id_usuario=?`, [idFilial, idUsuario]);
...
const listarDespesas = (idEmpresa, idFilial = null, status = null) => {
  ...
  if (idFilial) { filter += `AND d.id_filial = ? `; params.push(idFilial); }
  if (status) { filter += `AND d.status = ? `; params.push(status); }

  return execQuery(`
    SELECT d.id_despesa, d.descricao, d.preco, d.status, d.data_pag, f.nome as nome_fornecedor, fil.nome as nome_filial, d.data_criacao
...
const registrarDespesaFornecedor = (idEmpresa, d) => execQuery(
  `INSERT INTO despesas (id_empresa, id_fornecedor, id_filial, descricao, preco, status, data_pag, data_criacao) VALUES (?, ?, ?, ?, ?, 'pendente', ?, NOW())`,
...
const obterDetalhesFluxoPorFilial = async (idEmpresa, periodo) => {
  const sql = `
    SELECT f.id_filial, f.nome as nome_filial, COALESCE(SUM(v.total), 0) as receitas,
    ...
    FROM filiais f LEFT JOIN vendas v ON f.id_filial = v.id_filial AND ${getWhereData(periodo, 'data_venda', 'v')}
    WHERE f.id_empresa = ? GROUP BY f.id_filial, f.nome ORDER BY receitas DESC`;
  return await execQuery(sql, [idEmpresa, idEmpresa]);
};

const obterComparativoLojas = async (idEmpresa, periodo) => {
  const sql = `
    SELECT f.id_filial, f.nome as nome_filial, COUNT(v.id_venda) as total_vendas, COALESCE(SUM(v.total), 0) as valor_vendas,
    ...
    WHERE f.id_empresa = ? GROUP BY f.id_filial, f.nome ORDER BY valor_vendas DESC`;
  return await execQuery(sql, [idEmpresa]);
};
